...
def minio_upload_file(file_obj, file_name: str) -> Optional[str]:
    """
    上传文件到 MinIO
    ...
    try:
        ...
        # 路径前缀处理
        path_prefix = getattr(settings, 'MINIO_PATH_PREFIX', '')
        if path_prefix:
        ...
        # 生成访问URL
        file_url = minio_get_file_url(object_name)
        return file_url
        ...
        print(f"MinIO upload unexpected error: {e}")
        return None


def minio_delete_file(file_url: str) -> bool:
    """
    从 MinIO 删除文件
    ...
    try:
        # 从URL中提取 object_key
        object_key = file_url
        if settings.MINIO_ENDPOINT in file_url:
            # 提取 /bucket/key 部分
            parts = file_url.split(settings.MINIO_BUCKET + '/')
            if len(parts) > 1:
                object_key = parts[-1]
            else:
...
def minio_get_file_url(object_key: str) -> str:
    """
    获取文件访问URL
