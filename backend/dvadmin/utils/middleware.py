...
class ApiLoggingMiddleware(MiddlewareMixin):
    ...
    def __handle_response(self, request, response):
        ...
        # 记录日志不应影响业务接口返回；同时兼容历史脏数据导致的重复id问题
        try:
            operation_log, _created = OperationLog.objects.update_or_create(defaults=info, id=log_id)
        except MultipleObjectsReturned:
            OperationLog.objects.filter(id=log_id).update(**info)
            operation_log = (
                OperationLog.objects.filter(id=log_id)
        ...
        if operation_log and (not operation_log.request_modular) and settings.API_MODEL_MAP.get(request.request_path, None):
            operation_log.request_modular = settings.API_MODEL_MAP[request.request_path]
            operation_log.save(update_fields=['request_modular', 'update_datetime'])
