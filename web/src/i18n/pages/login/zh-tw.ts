// 定义内容
export default {
	label: {
		one1: '用戶名登入',
		two2: '手機號登入',
		three3: '註冊賬號',
		changePwd: '密码修改',
	},
	link: {
		one3: '協力廠商登入',
		two4: '友情連結',
	},
	account: {
		accountPlaceholder1: '請輸入登入賬號',
		accountPlaceholder2: '請輸入登入密碼',
		accountPlaceholder3: '請輸入驗證碼',
		accountPlaceholder4: '請輸入新密碼',
		accountPlaceholder5: '請再次輸入新密碼',
		accountBtnText: '登入',
	},
	mobile: {
		placeholder1: '請輸入手機號',
		placeholder2: '請輸入驗證碼',
		codeText: '獲取驗證碼',
		btnText: '登入',
		msgText: '* 溫馨提示：建議使用穀歌、Microsoft Edge，版本79.0.1072.62及以上瀏覽器，360瀏覽器請使用極速模式',
	},
	register: {
		registerPlaceholder1: '請輸入賬號',
		registerPlaceholder2: '請輸入密碼',
		registerPlaceholder3: '請輸入確認密碼',
		registerPlaceholder4: '請輸入昵称',
		registerPlaceholder5: '請輸入郵箱',
		registerPlaceholder6: '請輸入驗證碼',
		registerBtnText: '注 冊',
	},
	scan: {
		text: '打開手機掃一掃，快速登錄/注册',
	},
	signInText: '歡迎回來！',
};
