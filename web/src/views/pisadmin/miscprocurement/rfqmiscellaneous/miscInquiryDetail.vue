<template>
  <fs-page class="rfq-inquiry-detail-page" v-loading="pageLoading">
    <template #header>
      <header class="rfq-detail-toolbar">
        <div class="rfq-detail-toolbar__left">
          <el-button text class="rfq-detail-back" @click="goBack">
            <el-icon class="rfq-detail-back__icon"><ArrowLeft /></el-icon>
            返回
          </el-button>
          <h1 class="rfq-detail-toolbar__title">{{ pageTitle }}</h1>
        </div>
        <div class="rfq-detail-toolbar__meta">
          <span class="rfq-detail-toolbar__meta-label">状态</span>
          <el-tag :type="statusTagType(form.status)" effect="light" round>
            ...
            <el-form-item v-if="isInquiryBuyingMethod" label="报价截止时间" required>
              <div class="quote-deadline-input">
                <el-date-picker
                  ...
                  style="max-width: 100%"
                />
                <el-select v-model="quoteDeadlineTimeSlot" placeholder="时间" :disabled="isViewMode || !quoteDeadlineDate" style="max-width: 120px">
            ...
            <el-form-item v-if="!isInquiryBuyingMethod" label="投标开始时间" required>
              <div class="quote-deadline-input">
                <el-date-picker
                  ...
                  style="max-width: 100%"
                />
                <el-select v-model="bidStartTimeSlot" placeholder="时间" :disabled="isViewMode || !bidStartTimeDate" style="max-width: 120px">
                  <el-option v-for="slot in THIRTY_MIN_TIME_SLOTS" :key="slot" :label="slot" :value="slot" />
                </el-select>
              </div>
            </el-form-item>
            ...
            <el-form-item v-if="!isInquiryBuyingMethod" label="投标截止时间" required>
              <div class="quote-deadline-input">
                ...
                type="date"
                  ...
                  style="max-width: 100%"
                />
                <el-select v-model="bidEndTimeSlot" placeholder="时间" :disabled="isViewMode || !bidEndTimeDate" style="max-width: 120px">
                  <el-option v-for="slot in THIRTY_MIN_TIME_SLOTS" :key="slot" :label="slot" :value="slot" />
                </el-select>
              </div>
            </el-form-item>
...
const pageMode = ref<'create' | 'edit' | 'view'>('create')
const currentId = ref<number | null>(null)
const isViewMode = computed(() => pageMode.value === 'view')

const pageTitle = computed(() => {
  if (pageMode.value === 'create') return '新增询价单'
  if (pageMode.value === 'view') return '查看询价单'
  return '编辑询价单'
})
...
const saveForm = async () => {
  ...
  if (pageMode.value === 'edit' && !isOpenStatus(form.status)) {
  ...
  try {
    ...
    if (pageMode.value === 'create') {
      ...
      await api.UpdateObj({ ...payload, id: currentId.value })
    }
    try {
...
const loadPage = async () => {
  ...
  try {
    ...
    if (!Number.isFinite(numId)) {
      ElMessage.error('无效的询价单')
      router.back()
      return
    }
    const mode = qMode === 'edit' ? 'edit' : 'view'
    await openDetail({ id: numId }, mode)
  } finally {
...
<style scoped>
/* fs-page 为绝对定位铺满主区；header/slot 由组件分区，footer 固定在底部不随内容滚动 */
.rfq-inquiry-detail-page {
  box-sizing: border-box;
}
.rfq-inquiry-detail-page :deep(.fs-page-header) {
  border-bottom: none;
  padding: 0;
  flex-shrink: 0;
}
...
.rfq-detail-back:hover {
  color: var(--el-color-primary);
}
.rfq-detail-back__icon {
  margin-right: 2px;
  vertical-align: middle;
}
.rfq-detail-toolbar__title {
  margin: 0;
  font-size: 18px;
  font-weight: 600;
...
.rfq-detail-toolbar__meta {
  display: flex;
  align-items: center;
  gap: 8px;
  flex-shrink: 0;
}
.rfq-detail-toolbar__meta-label {
  font-size: 13px;
...
.rfq-inquiry-detail-page :deep(.fs-page-footer) {
  flex-shrink: 0;
