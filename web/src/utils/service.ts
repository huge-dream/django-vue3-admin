...
function createRequestFunction(service: any) {
	return function (config: any) {
		...
		// 兼容配置 baseURL=.../api 与接口 url=/api/... 的重复拼接，避免出现 /api/api/xxx
		try {
			const base = String(configDefault.baseURL ?? '');
			const url = String(configDefault.url ?? '');
			if (base.replace(/\/$/, '').endsWith('/api') && url.startsWith('/api/')) {
				configDefault.url = url.replace(/^\/api/, '');
			}
		} catch (_) {
			// ignore
		}
